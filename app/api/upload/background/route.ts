...
export async function POST(request: NextRequest) {
  try {
    ...
    const fileName = `background-${Date.now()}.${fileExtensionRaw}`;
    
    // Path to public/background directory
    const publicDir = join(process.cwd(), "public", "background");
    ...
    // Save file to public/background
    ...
    const publicUrl = `/background/${fileName}`;
